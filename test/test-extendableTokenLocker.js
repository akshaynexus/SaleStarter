...
describe("ExtendableTokenLocker", function () {
  ...
  beforeEach(async function () {
    const ExtendableTokenLocker = await ethers.getContractFactory("ExtendableTokenLocker");
    ...
    locker = await ExtendableTokenLocker.deploy(token.address,owner.address, secondsSinceEpoch + 2000);
  });
  ...
  it("Should not be able to sweep the locked token", async function () {
    expect(locker.sweep(token.address)).to.be.revertedWith('Cant sweep config token');
  })
  it("Should be able to sweep a token which has been sent to contract", async function () {
    ...
    expect(await spamToken.balanceOf(owner.address)).to.equal(await spamToken.totalSupply())
  })
});
