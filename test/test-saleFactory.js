...
describe("SaleFactory", function () {
  ...
  beforeEach(async function () {
    try {
      ...
      expect(await saleFactory.feeReceiver()).to.equal(
        await saleFactory.owner()
      );
  ...
  it("Should get enough allocation per eth", async function () {
    try {
      ...
      for (let i = 1; i < 5; i++) {
        ...
        expect(
          mockSale.connect(buyerWallets[i]).getRefund()
  ...
  it("Should get refund if sale doesnt pass softcap", async function () {
    try {
      ...
      for (let i = 1; i < 5; i++) {
        let startBal = await ethers.provider.getBalance(
          buyerWallets[i].address
        );
